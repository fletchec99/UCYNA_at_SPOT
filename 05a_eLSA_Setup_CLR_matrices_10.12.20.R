...
#I. Read in files-------
#Read in interpolated, CLR transformed data from a) euks in the AE filter, b) proks in the AE filter, c) proks in the durapore filters for both 1. 5m (surface), 2. DCM
...
#Euks first
euks_AE_5m <- read.table("int_CLR_data/euks_AE_5m_int_CLR_10.09.2020.tsv", sep="\t", header=T, stringsAsFactors = F, row.names=1) #Make ASV hashes equal to rownames
dim(euks_AE_5m) #28402 125
head(colnames(euks_AE_5m))

#Then proks AE
proks_AE_5m <- read.table("int_CLR_data/proks_AE_5m_int_CLR_10.08.2020.tsv", header=T, stringsAsFactors = F, row.names=1)
dim(proks_AE_5m) #78953 125
head(colnames(proks_AE_5m))

#Last, proks durapore
proks_D_5m <- read.table("int_CLR_data/proks_D_5m_int_CLR_10.08.2020.tsv", header=T, stringsAsFactors = F, row.names=1)
...
#2. DCM samples-----
#Euks first
euks_AE_DCM <- read.table("int_CLR_data/euks_AE_DCM_int_CLR_10.09.2020.tsv", sep="\t", stringsAsFactors = F, header=T, row.names=1)
dim(euks_AE_DCM) #28402 125
head(colnames(euks_AE_DCM))

#Proks AE
proks_AE_DCM <- read.table("int_CLR_data/proks_AE_DCM_int_CLR_10.08.20.tsv", sep="\t", stringsAsFactors = F, header=T, row.names=1)
dim(proks_AE_DCM) #78953 125
head(colnames(proks_AE_DCM))

#Proks durapore
proks_D_DCM <- read.table("int_CLR_data/proks_D_DCM_int_CLR_10.08.2020.tsv", sep="\t", stringsAsFactors = F, header=T, row.names=1)
...
tail(eLSA_in_5m$SPOT.2008.03.01.5m.AE)
...
#Write it out!
write.table(x=writeout_eLSA_in_5m, file="eLSA_input/eLSA_input_5m_CLR_abund_and_lit_10.12.2020.tsv", sep="\t", quote=F, row.names=F)
...
} #OK much better! A lot of the CLR abundances are zero again. 
...
#Write it out! 
write.table(x=writeout_eLSA_in_DCM, file="eLSA_input/eLSA_input_DCM_CLR_abund-lit_10.13.2020.tsv", sep="\t", quote=F, row.names=F)
